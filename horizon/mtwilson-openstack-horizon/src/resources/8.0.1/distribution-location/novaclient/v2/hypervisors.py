...
class HypervisorManager(base.ManagerWithFind):
    ...
    def hvspecs(self):
        """
        Get the metadata for all hypervisors.
        """
        return self._list("/os-hypervisors/hvspecs",
                          "hv_metadata_list")

    def truststatus(self, hypervisor):
        """
        Get the trust report for a specific hypervisor
        """
        return self._get("/os-hypervisors/%s/truststatus" % base.getid(hypervisor),
                         "trust_report")

    def asset_tags(self):
        """
        Get the asset tags available in nova database.
        """
        return self._get("/os-hypervisors/asset_tags",
                          "asset_tags")

    def metadata(self, hypervisor):
        """
        Get the metadata for a specific hypervisor
        """
        return self._get("/os-hypervisors/%s/metadata" % base.getid(hypervisor),
                         "hv_metadata")
