...
# OpenStack Horizon uninstall script
# Outline:
...
. $HORIZON_EXT_HOME/bin/functions.sh

# source patch-util script
. $HORIZON_EXT_HOME/bin/patch-util.sh

source $HORIZON_EXT_ENV/horizon-ext-layout > /dev/null 2>&1
...
# delete HORIZON_EXT_HOME
if [ -d $HORIZON_EXT_HOME ]; then
  rm -rf $HORIZON_EXT_HOME 2>/dev/null
fi

echo_success "Horizon uninstall complete"
