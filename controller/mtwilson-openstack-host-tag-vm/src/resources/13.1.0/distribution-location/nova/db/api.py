...
def compute_node_search_by_hostip(context, host_ip):
    """Get compute nodes by hypervisor hostip.

    :param context: The security context
    :param hypervisor_match: The hypervisor hostip

    :returns: List of dictionary-like objects each containing compute node
              properties
    """
    return IMPL.compute_node_search_by_hostip(context, host_ip)
...
def hvspec_create(context, values):
    """Create a compute node from the values dictionary."""
    return IMPL.hvspec_create(context, values)

def hvspec_update(context, hvspec_id, values):
    """Set the given properties on a compute node and update it."""
    return IMPL.hvspec_update(context, hvspec_id, values)

def hvspec_delete(context, hvspec_id):
    """Delete a compute node from the database."""
    return IMPL.hvspec_delete(context, hvspec_id)

def hvspec_get(context, hvspec_id):
    """Get a hypervisor metadata by its id."""
    return IMPL.hvspec_get(context, hvspec_id)

def hvspec_get_all(context):
    """Get hypervisor metadata for all hosts."""
    return IMPL.hvspec_get_all(context)

def hvspec_get_by_compute_node_id(context, compute_node_id):
    """Get a hypervisor metadata by compute_node_id."""
    return IMPL.hvspec_get_by_compute_node_id(context, compute_node_id)

def hvspec_get_by_key(context, key):
    """Get a specific metadata for all hosts."""
    return IMPL.hvspec_get_by_key(context, key)

def hvspec_get_by_compute_node_id_and_key(context, compute_node_id, key):
    """Get a hypervisor metadata by compute_node_id and key."""
    return IMPL.hvspec_get_by_compute_node_id_and_key(context, compute_node_id, key)
