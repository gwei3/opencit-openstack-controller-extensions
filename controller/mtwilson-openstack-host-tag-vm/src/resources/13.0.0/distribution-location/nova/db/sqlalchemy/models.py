...
class HVMetadata(BASE, NovaBase, models.SoftDeleteMixin):
    """Represents a metadata key/value pair for a hypervisor."""

    __tablename__ = 'hv_specs'
    __table_args__ = (
         schema.UniqueConstraint(
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    compute_node_id = Column(Integer, ForeignKey('compute_nodes.id'), nullable=False)
    key = Column(String(255), nullable=False)
    value = Column(Text)
