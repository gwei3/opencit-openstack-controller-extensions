...
class HostAPI(base.Base):
    ...
    def compute_node_search_by_hostip(self, context, host_ip):
        return objects.ComputeNodeList.get_by_hostip(context,
                                                         host_ip)
...
class HVMetadataAPI(base.Base):
    """Subset of the Compute Manager API for managing hypervisor metadata."""

    @wrap_exception()
    def create_hv_spec(self, context, compute_node_id,
                        key, value):
        """Create a new hypervisor metadata."""

        hvspec = objects.HVMetadata(context)
        hvspec.compute_node_id = compute_node_id
        hvspec.key = key
        hvspec.value = value
        hvspec.create()
    ...
    @wrap_exception()
    def update_hv_spec(self, context, id, compute_node_id,
                        key, value):
        """Update an existing hypervisor metadata."""
    ...
    @wrap_exception()
    def delete_hv_spec(self, context, hvspec_id):
        """Delete a hypervisor metadata by id."""
        objects.HVMetadata.destroy_by_id(context, hvspec_id)

    def get_hv_spec(self, context, hvspec_id):
        """Get a hypervisor metadata by id."""
        return objects.HVMetadata.get_by_id(context, hvspec_id)

    def get_hv_specs(self, context):
        """List metadata of all hypervisors."""
        return objects.HVMetadataList.get_all(context)

    def get_hv_specs_by_compute_node_id(self, context, compute_node_id):
        """Get a hypervisor metadata by compute_node id."""
        return objects.HVMetadataList.get_by_compute_node_id(context,
                                                         compute_node_id)

    def get_hv_specs_by_key(self, context, key):
        """Get a specific metadata of all hypervisors."""
        return objects.HVMetadataList.get_by_key(context,
                                                         key)

    def get_hv_spec_by_compute_node_id_and_key(self, context, compute_node_id, key):
        """Get a specific metadata for given compute_node."""
        return objects.HVMetadata.get_by_compute_node_id_and_key(context,
                                                         compute_node_id, key)
