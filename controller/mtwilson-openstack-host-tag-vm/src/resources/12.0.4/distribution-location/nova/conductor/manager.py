...
class ConductorManager(manager.Manager):
    ...
    # NOTE(hanlind): This method can be removed in version 3.0 of the RPC API
    def hvspec_create(self, context, values):
        result = self.db.hvspec_create(context, values)
        return jsonutils.to_primitive(result)

    # NOTE(hanlind): This can be removed in version 3.0 of the RPC API
    def hvspec_update(self, context, node, values):
        result = self.db.hvspec_update(context, node['id'], values)
        return jsonutils.to_primitive(result)

    # NOTE(hanlind): This can be removed in version 3.0 of the RPC API
    def hvspec_delete(self, context, node):
        result = self.db.hvspec_delete(context, node['id'])
        return jsonutils.to_primitive(result)
